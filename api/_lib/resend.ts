...
function resetPasswordHtml(name: string, resetLink: string): string {
  const first = firstName(name, "there");
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"/><meta name="viewport" content="width=device-width"/>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td style="background:#ffffff;padding:40px 28px 34px;">
          <table width="100%" cellpadding="0" cellspacing="0">
            ...
            <tr><td style="padding-bottom:28px;text-align:center;">
              <p style="margin:0;font-size:15px;color:#4a4a4a;line-height:1.65;">
                Hey ${first}, we got a request to reset the password for your Popcorn account. Click the button below to choose a new one.
              </p>
            </td></tr>
            <tr><td style="padding-bottom:28px;text-align:center;">
              <a href="${resetLink}" style="display:inline-block;padding:14px 38px;background:#042c85;color:#fff1cd;font-size:13px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;text-decoration:none;border-radius:999px;">
                RESET PASSWORD
              </a>
            </td></tr>
            <tr><td style="padding-bottom:18px;text-align:center;">
              <p style="margin:0;font-size:13px;color:#9aa0a8;line-height:1.6;">
                This link expires in 1 hour. Didn't request this? You can safely ignore this email — your password won't change.
              </p>
            </td></tr>
            <tr><td style="text-align:center;">
              <p style="margin:0;font-size:12px;color:#9aa0a8;line-height:1.6;">
                If the button doesn't work, copy and paste this link:<br/>
                <a href="${resetLink}" style="color:#042c85;word-break:break-all;">${resetLink}</a>
              </p>
            </td></tr>
          </table>
        </td></tr>
...
export async function sendResetPasswordEmail(
  ...
  try {
    const result = await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: "Reset your Popcorn password",
    ...
    console.log(`Reset-password email sent to ${email}`, result.data);
    return { success: true };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error("Failed to send reset-password email:", message);
    return { success: false, error: message };
  }
}
